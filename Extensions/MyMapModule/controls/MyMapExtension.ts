...
export class MyMapClass extends IControl {
    ...
    public initialize(props: any): any {
        ...
        if (this.definition().data.ExtensionProperties.Prop) {
            ...
            this._customerInfo.mapType = property.MapType;
        }
    ...
    private onActivityPaused(args) {
        ...
        Dialogs.confirm("onActivityPaused").then(result => {
    ...
    private onActivityResumed(args) {
        ...
        Dialogs.confirm("onActivityResumed").then(result => {
            console.log("Dialog result: " + result);
        });
        if (!this._mapView || this != args.activity) return;
        this._mapView.onResume();
                ...
                if (app.ios) {
                    return this.getlatlang(customerAddress)
                        .then((cordinates) => {
                            ...
                            annotation.title = `${this._customerInfo.city} - Sales in last 90 days`;
                            annotation.subtitle = "";
                            // if(this._customerInfo.city=="Brazil"){
                            // this._mapView.mapType =  MKMapTypeHybrid;
                            //             }else{
                            //                 this._mapView.mapType =  MKMapTypeSatellite;

                            //             }

                            switch (this._customerInfo.lastName) {
                                case "Standard":
                                    this._mapView.mapType = MKMapTypeStandard;
                                    break;
                                case "Satellite":
                                    this._mapView.mapType = MKMapTypeSatellite;
                                    break;
                                ...
                                case "Satellite Flyover":
                                    this._mapView.mapType = MKMapTypeSatelliteFlyover

                                    break;
                                case "Hybrid Flyover":
                                    this._mapView.mapType = MKMapTypeHybridFlyover

                                    break;
                                case "Muted Standard":
                                    this._mapView.mapType = MKMapTypeMutedStandard
